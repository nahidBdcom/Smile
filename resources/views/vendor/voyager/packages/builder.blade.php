...
@section('page_title', __('voyager::generic.package_builder'))

@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i>Package FAQ Builder ({{ $package->title }})
        <div class="btn btn-success add_item"><i class="voyager-plus"></i>New Package FAQ</div>
    </h1>
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">Drag and drop the package FAQs below to re-arrange them.</p>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="voyager-trash"></i>Are you sure you want to delete this Package FAQ?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.packages.item.destroy', ['package' => $package->id, 'id' => '__id']) }}"
                          id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                               value="Yes, Delete This Package FAQ">
                    </form>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('voyager::generic.close') }}"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 id="m_hd_add" class="modal-title hidden"><i class="voyager-plus"></i> Create a New Package FAQ</h4>
                    <h4 id="m_hd_edit" class="modal-title hidden"><i class="voyager-edit"></i> Edit Package FAQ</h4>
                </div>
                <form action="" id="m_form" method="POST"
                      data-action-add="{{ route('voyager.packages.item.add', ['package' => $package->id]) }}"
                      data-action-update="{{ route('voyager.packages.item.update', ['package' => $package->id]) }}">

                    <input id="m_form_method" type="hidden" name="_method" value="POST">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <div>
                            <label for="title">Title of the Package FAQ</label>
                        ...
                        <input type="hidden" id="m_package_id" name="package_id" value="{{ $package->id }}">
    ...
    <script>
        $(document).ready(function () {
            @if ($isModelTranslatable)
                 ...
                 */
                $('#package_faq_modal').multilingual({
                    "form":          'form',
                    "transInputs":   '#package_faq_modal input[data-i18n=true]',
            ...
            $m_modal.on('show.bs.modal', function(e, data) {
                ...
                if (_adding) {
                    ...
                    $m_package_id.val(_src.data('package_id'));
                    $m_id.val(id);
            ...
            $('.item_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_form')[0].action = '{{ route('voyager.packages.item.destroy', ['package' => $package->id, 'id' => '__id']) }}'.replace('__id', id);
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('voyager.packages.order_item',['package' => $package->id]) }}', {
